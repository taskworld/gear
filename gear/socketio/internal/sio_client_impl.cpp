...
void client_impl::connect(const string& uri, const map<string, string>& query) {
  ...
  m_client.get_io_service().dispatch(
      std::bind(&client_impl::connect_impl, this, uri, m_query_string));
...
void client_impl::close() {
  ...
  m_client.get_io_service().dispatch(
      std::bind(&client_impl::close_impl, this, close::status::normal, "End by user"));
}
...
asio::io_service& client_impl::get_io_service() { return m_client.get_io_service(); }
...
void client_impl::timeout_pong(const std::error_code& ec) {
  ...
  m_client.get_io_service().dispatch(
      std::bind(&client_impl::close_impl, this, close::status::policy_violation, "Pong timeout"));
}
...
void client_impl::timeout_reconnect(std::error_code const& ec) {
  ...
  if (m_con_state == con_closed) {
    ...
    m_client.get_io_service().dispatch(
        std::bind(&client_impl::connect_impl, this, m_base_url, m_query_string));
  }
}
...
void client_impl::on_fail(connection_hdl con) {
  ...
  if (m_reconn_made < m_reconn_attempts) {
    ...
    m_reconn_timer.reset(new system_timer(m_client.get_io_service()));
    std::error_code ec;
...
void client_impl::on_handshake(message::ptr const& message) {
  if (message && message->get_flag() == message::flag_object) {
    ...
    m_ping_timer.reset(new system_timer(m_client.get_io_service()));
    std::error_code ec;
...
failed:
  // just close it.
  m_client.get_io_service().dispatch(std::bind(&client_impl::close_impl, this,
                                               close::status::policy_violation, "Handshake error"));
}
