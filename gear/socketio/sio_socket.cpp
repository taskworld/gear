...
using system_timer = asio::basic_waitable_timer<std::chrono::system_clock>;
...
class event_adapter {
 public:
  ...
  static inline event create_event(std::string const& nsp, std::string const& name,
                                   message::list&& message, bool need_ack) {
...
void event::put_ack_message(message::list const& ack_message) {
  if (m_need_ack) m_ack_message = std::move(ack_message);
}

inline event::event(std::string const& nsp, std::string const& name, message::list&& messages,
                    bool need_ack)
    : m_nsp(nsp), m_name(name), m_messages(std::move(messages)), m_need_ack(need_ack) {}

inline event::event(std::string const& nsp, std::string const& name, message::list const& messages,
                    bool need_ack)
...
inline message::list& event::get_ack_message_impl() { return m_ack_message; }
...
#undef SYNTHESIS_SETTER
 ...
 private:
  ...
  void on_socketio_event(const std::string& nsp, int msgId, const std::string& name,
  ...
  std::unique_ptr<system_timer> m_connection_timer;
...
void socket::impl::on_error(error_listener const& l) { m_error_listener = l; }

void socket::impl::off_error() { m_error_listener = nullptr; }

socket::impl::impl(client_impl* client, std::string const& nsp)
    : m_client(client), m_nsp(nsp), m_connected(false) {
  NULL_GUARD(client);
  if (m_client->opened()) {
    send_connect();
  }
...
void socket::impl::emit(std::string const& name, message::list const& msglist,
                        std::function<void(message::list const&)> const& ack) {
  NULL_GUARD(m_client);
...
void socket::impl::send_connect() {
  ...
  m_client->send(p);
  m_connection_timer.reset(new system_timer(m_client->get_io_service()));
  std::error_code ec;
  m_connection_timer->expires_from_now(std::chrono::milliseconds(20000), ec);
  m_connection_timer->async_wait(
      std::bind(&socket::impl::timeout_connection, this, std::placeholders::_1));
}
...
void socket::impl::on_open() { send_connect(); }
      ...
      case packet::type_binary_event: {
        ...
        if (ptr->get_flag() == message::flag_array) {
          ...
          if (array_ptr->get_vector().size() >= 1 &&
              array_ptr->get_vector()[0]->get_flag() == message::flag_string) {
            const string_message* name_ptr =
                static_cast<const string_message*>(array_ptr->get_vector()[0].get());
            message::list mlist;
            for (size_t i = 1; i < array_ptr->get_vector().size(); ++i) {
              mlist.push(array_ptr->get_vector()[i]);
            }
            this->on_socketio_event(p.get_nsp(), p.get_pack_id(), name_ptr->get_string(),
                                    std::move(mlist));
          }
        }
...
void socket::impl::on_socketio_event(const std::string& nsp, int msgId, const std::string& name,
                                     message::list&& message) {
...
socket::socket(client_impl* client, std::string const& nsp) : m_impl(new impl(client, nsp)) {}
...
void socket::off(std::string const& event_name) { m_impl->off(event_name); }

void socket::off_all() { m_impl->off_all(); }

void socket::close() { m_impl->close(); }

void socket::on_error(error_listener const& l) { m_impl->on_error(l); }

void socket::off_error() { m_impl->off_error(); }
...
std::string const& socket::get_namespace() const { return m_impl->get_namespace(); }

void socket::on_connected() { m_impl->on_connected(); }

void socket::on_close() { m_impl->on_close(); }

void socket::on_open() { m_impl->on_open(); }

void socket::on_message_packet(packet const& p) { m_impl->on_message_packet(p); }

void socket::on_disconnect() { m_impl->on_disconnect(); }
}
