...
class event {
 ...
 protected:
  event(std::string const& nsp, std::string const& name, message::list const& messages,
        bool need_ack);
...
class socket {
 public:
  typedef std::function<void(const std::string& name, message::ptr const& message, bool need_ack,
